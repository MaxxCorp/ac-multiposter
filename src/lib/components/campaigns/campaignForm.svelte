<script lang="ts">
    ...
    import type { Campaign } from "../../../routes/campaigns/list.remote";
...
<div class="max-w-2xl mx-auto">
    <Breadcrumb
        feature="campaigns"
        current={campaign?.name ?? "New Campaign"}
    />
    <div class="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        >
            ...
            <label class="block">
                ...
                >
                <input
                    {...remoteFunc.fields.name.as("text")}
                    class="mt-2 w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 {(remoteFunc.fields.name.issues()
                        ?.length ?? 0) > 0
                        ? 'border-red-500'
                        : 'border-gray-300'}"
            ...
            <label class="block">
                ...
                >
                <textarea
                    {...remoteFunc.fields.content.as("text")}
                    rows="12"
                    class="mt-2 w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono text-sm {(remoteFunc.fields.content.issues()
                        ?.length ?? 0) > 0
                        ? 'border-red-500'
                        : 'border-gray-300'}"
                    placeholder={"{}"}
