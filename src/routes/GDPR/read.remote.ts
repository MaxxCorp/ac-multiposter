...
export const readContent = query(readContentSchema, async (params) => {
    ...
    console.log(`[GDPR read.remote] Resolved lang: ${lang || 'en'}, branch: ${branch}`);

    try {
        const result = await getContent('gdpr', 'main', lang || 'en', branch);
        console.log('[GDPR read.remote] Result block:', result?.block?.name);
        return result;
    } catch (e) {
        console.error('[GDPR read.remote] Error:', e);
        throw e;
    }
});
